...
bool FWaitFlow = FALSE;					// para indicar que debo esperar por el Flow control
bool FWaitLongRespBSize = FALSE;		// para indicar que debo esperar por el Flow control
bool FlagTP = FALSE;
int Counter_BSize;
BOOL g_bStopSelectedMsgTx = TRUE; 
static CBaseDIL_CAN* g_pouDIL_CAN_Interface;
static DWORD g_dwClientID = 0;
int c_unPreviousTime =-1;	 //Deberia ir en otra clase para que sea más elegante
double c_dDiffTime=0;		//Deberia ir en otra clase para que sea más elegante
int S3_Client = 2000; 
...
void CUDSMainWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_DIAG_SERVICE, m_omDiagService);	 // Duda de como deben estar definidos
...
{	// omByteStr contains the list of bytes that has to be sent in the long request 
	...
	c_numberOfTaken = c_numberOfTaken+2;	 // The consecutive Messages will contain one byte more that the FirstFrame
...
void CUDSMainWnd::PrepareFlowControl(){
	...
	Counter_BSize = BSize;								// BSize and SSTMin  depens of the value put in the settingsWnd. 
	FWaitLongRespBSize = Counter_BSize;					// Not used
	psTxCanMsgUds->m_psTxMsg->m_ucDataLen= SizeFC; 		//The DLC of the FCmessage depens of the value put in the settingsWnd.  
...
{	
	...
	m_nTimer = SetTimer(ID_TIMER_SEND_BUTTON,P2_Time /*TIME_UNABLE_SEND_BUTTON*/, NULL);	// Time to wait to able the send button again
		...
		case INTERFACE_EXTENDED_11:
			{
				...
				if (Result>6){								// Long Request - The limit of a simple request for extended is 6 because the byte that contains the TA. 
					...
					m_nTimer = SetTimer(ID_TIMER_SEND_BUTTON, P2_Time , NULL);			// Start Timer to wait to able by default the send button again
...
void  CUDSMainWnd::OnTimer(UINT_PTR nIDEvent){
	...
	if(nIDEvent ==ID_TIMER_TP && psTxCanMsgUds->m_psTxMsg != NULL) {		//Prepare the message
		...
		psTxCanMsgUds->m_psTxMsg->m_ucChannel = (UCHAR)m_omComboChannelUDS.GetCurSel()+1;
		psTxCanMsgUds->m_psTxMsg->m_unMsgID = (int)m_omCanID.lGetValue(); 
		memset(psTxCanMsgUds->m_psTxMsg->m_ucData, 0, 8);				//siempre poner todos los bytes a 0	
		if(fInterface == INTERFACE_EXTENDED_11 ) psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte-1]= TargetAddress;	
		psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte]= 0x02;			//Siempre el tester Present tendrá 2 bytes 0x3E y 0x00 
		psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte+1]= 0x3E;
		//psTxCanMsgUds->m_psTxMsg->m_unMsgID = MsgID;
...
BOOL CUDSMainWnd::OnInitDialog(){
	...
	m_omCheckTP.SetCheck(BST_UNCHECKED);  // Marcar como no check el TP
	vInitializeUDSfFields();			 //Algo así para inicializar todos los campos
	return TRUE;
}
//________________________________________________________________________________________________________________________________________________________________
//________________________________________________________________________________________________________________________________________________________________
...
void CUDSMainWnd::vInitializeUDSfFields(){   
	...
	m_omEditMsgData.vSetBase(BASE_HEXADECIMAL);   //Pendiente asunto de como avisar que es Hexa
	...
	m_omSourceAddress.vSetSigned(true); // Verificar por ue a TRUE? 
	//m_omSourceAddress.LimitText(2); //Esa correcto verdad! 
	m_omSourceAddress.vSetValue(SourceAddress);

	m_omTargetAddress.vSetBase(BASE_HEXADECIMAL);
	m_omTargetAddress.vSetSigned(true); // Verificar por ue a TRUE? 
	//m_omTargetAddress. LimitText(2); //Esa correcto verdad! 
	m_omTargetAddress.vSetValue(TargetAddress);

	switch (fInterface){					  // Para evaluar como deben estar los Textbox del SA y TA
			case INTERFACE_NORMAL_11:
				{
...
CUDSMainWnd* CUDSMainWnd::s_podGetUdsMsgManager()
{
	...
	{
		m_spodInstance = new CUDSMainWnd(0,0,INTERFACE_NORMAL_11,0);	   // Coloca el 3er parámetro a INTERFACE_NORMAL_11 solo por colocarlo ya que 
		/*	m_spodInstance->objPrue =1;*/								   // La creación de esta clase no me importa
